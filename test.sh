#!/bin/bash
set -e
hash=$(curl -X GET --header "Accept: */*" "${API_REQUEST}")
for run in $(seq 1 "${NUMBER_OF_LOOPS}")
do
  sleep 30
  result=$(curl -X GET --header "Accept: */*" "https://endtest.io/api.php?action=getResults&appId=${APP_ID}&appCode=${APP_CODE}&hash=${hash}&format=json")
